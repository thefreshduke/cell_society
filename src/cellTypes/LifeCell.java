...
public class LifeCell extends Cell {
    ...
    protected final static int THRESHOLD_FOR_LIFE = 3;
    protected final static int THRESHOLD_FOR_STATUS_QUO = 2;
     ...
     */
    protected LifeCell(int x, int y, int state, IEdgeStrategy edgeStrategy,
    ...
    public int countNumberOfLiveNeighbors() {
        int counter = 0;
        List<Cell> listOfNeighbors =
                super.calculateNeighbors(listOfCellsInGrid,
                        myXDelta, myYDelta);
    ...
    public void doAction() {
        int numberOfLiveNeighbors = countNumberOfLiveNeighbors();

        if (numberOfLiveNeighbors == THRESHOLD_FOR_LIFE) {
            myNextState = ALIVE;
        }
        else if (numberOfLiveNeighbors == THRESHOLD_FOR_STATUS_QUO) {
            myNextState = myState;
        }
        else {
            myNextState = DEAD;
