...
public class PredPreyCell extends Cell {
	...
	private boolean sharkWillEat = false; //just for fish
	...
	private void doSharkAction() {
		...
		if (neighbors.size() > 0) {
			...
			PredPreyCell destinationCell = (PredPreyCell) listOfCellsInGrid[newCell.myX][newCell.myY];
			if (newCell.myState == 1) { // if fish, eat it
				sharkEnergy += FISH_ENERGY;			
				destinationCell.sharkWillEat = true;
	...
	private ArrayList<Cell> calculateSharkNeighbors() {
		...
		for (int i = 0; i < neighbors.length; i++) {
			int newX = (myX + xDelta[i] + listOfCellsInGrid[0].length)
					% listOfCellsInGrid[0].length;
			int newY = (myY + yDelta[i] + listOfCellsInGrid.length)
					% listOfCellsInGrid.length;
